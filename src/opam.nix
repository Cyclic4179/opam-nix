...
let
  ...
  inherit (import ./evaluator lib) compareVersions' getUrl fetchImpure;
  ...
  injectSources =
    ...
    if sourceMap ? ${def.name} then
      ...
      def // { src = (bootstrapPackages.fetchgit { inherit (def.src) url rev hash; }) + def.src.subdir; }
    else
      def;
    ...
    let
      ...
      files =
        if recursive then
          ...
          (
            contents // optionalAttrs (contents.opam or null == "directory") { opam = readDir "${dir}/opam"; }
          );
      opamFiles = filterAttrsRecursive (name: value: isAttrs value || hasSuffix "opam" name) files;
      opamFilesOnly = converge (filterAttrsRecursive (_: v: v != { })) opamFiles;
  ...
  filterOpamRepo =
        ...
        mapAttrs (
          ...
          if version == "*" || isNull version then
            ...
            namePathPair "packages/${name}/${name}.${version}" (
              let
                path = "${repo}/packages/${name}/${name}.${version}";
              in
              if builtins.pathExists path then path else defaultPath
            )
        ) packages
      )
    )
    // optionalAttrs (repo ? passthru) {
      passthru =
        ...
        in
        repo.passthru // mapAttrs (_: pickRelevantVersions) { inherit (repo.passthru) sourceMap pkgdefs; };
    };
    ...
    let
      findPackage =
        ...
        let
          ...
          repos' = filter (
            repo: repo ? passthru.pkgdefs.${name}.${version} || pathExists (pkgDir repo)
          ) repos;
          repo = if length repos' > 0 then head repos' else throw "Could not find package ${name}.${version}";
        ...
        // optionalAttrs (pathExists (pkgDir repo + "/files")) { files = filesPath; }
  ...
  getPinDepends =
    ...
    if pkgdef ? pin-depends then
      map (
        ...
        in
        filterOpamRepo { ${name} = version; } (makeOpamRepo (fetchImpure (last dep) project))
      ) pkgdef.pin-depends
    else
      [ ];
