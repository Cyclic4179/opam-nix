...
let
  ...
  overrides = {
    ...
    ocaml = oa: { opam__ocaml_config__share = "${final.ocaml-config}/share/ocaml-config"; };
    ...
    feather = oa: { nativeBuildInputs = oa.nativeBuildInputs ++ [ final.nixpkgs.procps ]; };
    ...
    camlpdf = oa: { nativeBuildInputs = oa.nativeBuildInputs ++ [ final.nixpkgs.which ]; };
    ...
    coq-of-ocaml = oa: lib.optionalAttrs (lib.versionAtLeast oa.version "2.5.3") { sourceRoot = "."; };
