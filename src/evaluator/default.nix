...
rec {
  ...
  varToShellVar =
    ...
    let
      s = splitString ":" var;
    in
    concatMapStringsSep "__" (replaceStrings
      [
        "-"
        "+"
        "."
  ...
  fetchImpure =
    url: project:
    let
      splitUrl = splitString "+" (head (splitString ":" url));
    in
    let
      proto = if length splitUrl > 1 then head splitUrl else null;
    in
    if proto == "git" then
      fetchGitURL url
    else if proto == "http" || proto == "https" then
    ...
    then
      ...
      throw "[opam-nix] Protocol '${proto}' is not yet supported";
    ...
    let
      ...
      src =
        if pkgdef ? url then
          ...
          if hashes == { } then fetchImpure archive null else pkgs.fetchurl ({ url = archive; } // hashes)
        else
